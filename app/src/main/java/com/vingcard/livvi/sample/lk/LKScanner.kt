...
class LKScanner(context: Context): LKReachableDeviceListener, ScanLockCallback, CoroutineScope
{
    // Scanners
    ...
    override fun onReachableDevices(reachableDevices: List<LKCommDevice>)
    {
        launch(job) {
            _mutex.withLock {
                ...
                visibleDevices.removeAll { e -> e.kind ==  LKDeviceType.LIVVI }
        ...
        launch {
            ...
            _mutex.withLock {
                ...
                if (indexOfTTDeviceOnList == -1) {
                    this@LKScanner.visibleDevices.add(scanResult)
                    needsToNotify = true
                } else {
                    this@LKScanner.visibleDevices
        ...
        _timerJob = _tickerFlow(7000)
            .onEach {
                ...
                launch(job) {
                    ...
                    _mutex.withLock {
                        elementsWasRemoved = this@LKScanner.visibleDevices.removeAll {
